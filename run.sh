...
LATEST_ITERATION=$(ssh "$SERVER" "ls -td $REMOTE_DIR/output/$LATEST_FOLDER/point_cloud/*| head -n 1 | xargs -I {} basename {}")

echo "Latest iteration found: $LATEST_ITERATION"

# Step 6: Download the latest point_cloud.ply
REMOTE_PLY_PATH="$REMOTE_DIR/output/$LATEST_FOLDER/point_cloud/$LATEST_ITERATION/point_cloud.ply"
